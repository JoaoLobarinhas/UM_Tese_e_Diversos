    ...
    const loadData = () =>{
        ...
        SecureStore.getItemAsync("user_id").then((idUser)=>{
            if(idUser){
                setDataForList([])
                setIdUser(idUser)
                axiosInstance.get("/history/"+idUser).then((response)=>{
                    if(response.data.MESSAGE){
                        setMsgError("You don't have any movies on your history")
                    }
                    else{
    ...
    return (
       <SafeAreaView style={stylesGlobal.app}>
           ...
           <View style={styles.container}>
                <Text style={[styles.title,{color:colors.blue}]}>History</Text>
                ...
                <View style={[styles.containerList , {width:"92%", alignSelf:"center"}]}>
                    ...
                    :
                        <FlatList
                            ...
                            renderItem={({item}) => (
                                <CardMovieHistory idMovie={item.id_movie} title={item.title} image={item.profile_photo} navigation={props.navigation} idUser={idUser} year={item.release_year}/>
                            )}
