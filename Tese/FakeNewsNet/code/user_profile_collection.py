...
def get_user_ids_in_folder(samples_folder):
    ...
    for news_id in os.listdir(samples_folder):
        ...
        retweets_dir = "{}/{}/retweets".format(samples_folder, news_id)
        if is_folder_exists(news_dir) and is_folder_exists(tweets_dir):
            ...
            if is_folder_exists(retweets_dir):
                for retweet_file in os.listdir(retweets_dir):
                    retweet_object = json.load(open("{}/{}".format(retweets_dir, retweet_file)))
                    
                    if retweet_object["retweets"]:
                        for ids in retweet_object["retweets"]:
                            if not ids["user"]["id"] in user_ids:
                                user_ids.add(ids["user"]["id"])
