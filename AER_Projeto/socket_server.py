...
class InfoTransito:
    ...
    def __init__(self, lat, longi, info, state, origen, destiny):
        self.lat = lat
        self.longi = longi
        self.info = info
        self.state = state
...
def sender(closeProcess, routingTabel):
    ...
    while not closeProcess.is_set():
        ...
        if(switchBetweenProt):
            ...
            flag_with_table = [Flags.route_request,broadcastMSG]
            data = pickle.dumps(flag_with_table)
            s.sendto(data, (addrinfo[4][0], MYPORT))
            switchBetweenProt = True
...
def update_RT_Broadcast(routingTabel, data, lock):
    # Da update ao routing tabel com os dados que recebe do pacote de broadcast
    with lock:
...
def put_infos(infos, newInfo, lock):
    # Adiciona os dado a base de dados em chache
    with lock:
        notExist = True
        for info in infos:
            if(newInfo.lat == info.lat and newInfo.longi == info.longi and newInfo.info == info.info
            and newInfo.origen == info.origen and newInfo.destiny == info.destiny and newInfo.date == info.date):
                notExist=False
        if notExist:
            infos.append(newInfo)
    
def update_delete_RT(routingTabel,lock):
    # Elimina os dados que já expiraram da routing table
    with lock:
        for i in range(len(routingTabel) -1,-1,-1):
...
def check_info_NDN(messagesRecieved,info):
    ...
    if resp:
        ...
        info.message = resp
        info.jumpsLeft = _jumps - info.jumpsLeft
        sender_RR(info, Flags.ndn_resp_get_info)
    else:
        sender_RR(info,Flags.ndn_get_info)

def receiver(closeProcess, routingTabel):
        ...
        while not closeProcess.is_set():
            ...
            if data[0] is Flags.hello:
                ...
                if(sender != _ipv6):
                    ...
                    #verifica se é realmente o no que deve passar para voltar
                ...
                #if(data[1].lifeSpan >= int(time.time())):
                    if(data[1].message == _name):
                        data[1].jumpsLeft = _jumps
                        data[1].message = messageTabel
                ...
                #if(data[1].lifeSpan >= int(time.time())):
                    if(data[1].message.destiny == _name):
                        future.append(executer.submit(put_infos,messageTabel,data[1].message,_lockServer))
                    elif(data[1].lastPass != _ipv6 and data[1].jumpsLeft > 0):
                        data[1].jumpsLeft = data[1].jumpsLeft-1
                        data[1].lastPass = _ipv6
                    ...
                    if(data[1].lastPass != _ipv6 and data[1].jumpsLeft > 0):
                        data[1].jumpsLeft = data[1].jumpsLeft-1
                        data[1].passThrough.append(PassThroughObj(_ipv6,socket.gethostname()))
                        future.append(executer.submit(check_info_NDN,messageTabel,data[1]))
            elif data[0] is Flags.ndn_resp_get_info:
                #if(data[1].lifeSpan >= int(time.time())):
                    data[1].jumpsLeft = data[1].jumpsLeft-1
                    if(data[1].passThrough[data[1].jumpsLeft].ipv6 == _ipv6 and data[1].lastPass != _ipv6 and data[1].jumpsLeft > 0):
                        data[1].lastPass = _ipv6
...
if __name__ == "__main__":
    ...
    pReceiver = multiprocessing.Process(target=receiver, args=[closeProcess,routingTabel])
